...
class Simulator(object):
    ...
    def solve(self, m, waves, slit_x, slit_y):
        ...
        nxpix = self.det_dims[1]
        nypix = self.det_dims[0]
        dpix = self.dpix						# mm/pixel
        n = slit_x.size
        f_col_1 = self.f_col_1					# mm
        f_col_2 = self.f_col_2					# mm
        alpha_ech = self.alpha_ech				# deg
        blaze_ech = self.blaze_ech				# deg
        gamma_ech = self.gamma_ech				# deg
        sigma_ech =   1.0 / self.sigma_ech_inv	# lines/mm ? Conversion from Tino's excel should be in microns
        alpha_cd = self.alpha_cd				# micron
        sigma_cd = 1.0 / self.sigma_cd_inv		# micron
        f_cam = self.f_cam						# mm
        f_cam_1 = self.f_cam_1					# mm
        returnx = np.empty(1)
        returny = np.empty(1)
        ...
        ydm_0 = self.ydm_0						# mm
        ydr_0 = self.ydr_0						# mm
        tau_dl = self.tau_dl					# deg
        tau_dm = self.tau_dm					# deg
        tau_dr = self.tau_dr					# deg
