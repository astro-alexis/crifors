...
# =============================================================================
    ...
    def find_ccd_limits_solve(self, write=False, plot=True):
        # TODO SHOULD THIS BE A WAVEFUNC METHOD ?
        n = self.orders.size
        det_wl_lim = np.empty([self.ndet*2, n])
        ...
        for i,m in enumerate(self.orders):
            ...
            func(blaze_flag, return_mode, n, m, nxpix, nypix, f_col_1, f_col_2,
                ...
                ydm_0, ydr_0, tau_dl, tau_dm, tau_dr, slit_x, slit_y, wl,
                returnx, returny, returnwaves, returncounts)
            solvex[:, i] = returnx
            solvey[:, i] = returny
            interpx[:, i] = xmid
            interpy[:, i] = ymid
            ...
            wl = wl[inds]
            xpol=returnx[inds]
            fxmid = InterpolatedUnivariateSpline(xpol, wl)
            wlpmid = fxmid(x_det)
            det_wl_lim[:, i] = wlpmid
            print '\n wlpmid: ', wlpmid
